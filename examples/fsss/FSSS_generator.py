...
- Trajectory-level generation
- Switched linear latent dynamics
- Factorized discrete modes
- Continuous nuisance variable (load)
- Device-specific observation distortions
- Explicit transition metadata
- Window extraction built on full trajectories

Output files:
1. trajectories.csv  : time-step level table
2. windows.npz       : fixed windows + labels/metadata
3. metadata.json     : dataset summary and config
...
class GeneratorConfig:
    ...
    # Continuous nuisance factor range.
    load_min: float = 0.8
    load_max: float = 1.2
    ...
    # Transition sharpness. 0 means hard switch; >0 blends between two modes.
...
class FactorizedSwitchingGenerator:
    ...
    def extract_windows(self, trajectories: List[Dict[str, object]]) -> Dict[str, np.ndarray]:
        ...
        for traj in trajectories:
            ...
            for start in range(0, T - cfg.window_length + 1, cfg.stride):
                ...
                # Majority labels for factor probes.
    ...
    def _build_devices(self) -> List[Dict[str, np.ndarray]]:
        ...
        for device_id in range(cfg.num_devices):
            # Per-channel gain variation: mild and independent.
            ...
            # Keep device mixing very small so device does not compete with coupling.
    ...
    def _sample_segments(self, total_length: int) -> List[Dict[str, object]]:
        ...
        # Draw dwell lengths and rescale to fill the trajectory.
        ...
        # Fix final lengths to sum exactly to total_length.
    ...
    def _build_dynamics_matrix(self, spectral_name: str, coupling_name: str, load: float) -> np.ndarray:
        ...
        # Base mode-dependent 2x2 blocks.
        blocks = []
        ...
        # Load perturbs radius and angle, but should not completely redefine the mode.
        load_delta = load - 1.0
        ...
        # Coupling controls mixing strength.
        coupling_strength = {
            "low": 0.01,
            "medium": 0.05,
            "high": 0.1,
    ...
    def _build_dataset_metadata(self, trajectories: List[Dict[str, object]], windows: Dict[str, np.ndarray]) -> Dict[str, object]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate factorized switched state-space benchmark.")
