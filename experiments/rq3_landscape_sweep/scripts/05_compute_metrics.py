"""Step 5: Compute trace metrics for all (scenario, model) cells.

Run directly on the login node (~5 min for all 56 cells).

Usage:
  python 05_compute_metrics.py --data_dir ../results/data \
...
def _build_trajectory_data(emb_data: Dict, stride: int):
    """Split flat embeddings+labels into per-trajectory structures for metric computation."""
    embeddings = emb_data["embeddings"]
    mode_ids = emb_data["mode_id"]
    traj_ids = emb_data["trajectory_id"]
    window_starts = emb_data["window_start"]
    is_trans = emb_data["is_transition_window"]
    ...
    for tid in unique_trajs:
        ...
        # Extract contiguous segments for loop consistency
    ...
    # Find change point windows in the full flat sequence
    all_emb = embeddings
    all_modes = mode_ids
    ...
    return all_emb, all_modes, change_point_windows, per_traj_embs, per_traj_modes, per_traj_segs
...
def main() -> None:
    ...
    print(f"Computing metrics for {len(scenario_ids)} scenarios × {len(models)} models = {total} cells")
