"""SynIOL: Synthetic Industrial Operational Log generation."""
...
BASE_EVENT_PROBS: Dict[str, float] = {
    "STATUS_CHECK": 0.14,
    "VALUE_READ": 0.14,
    "SETPOINT_UPDATE": 0.05,
    "CONTROL_COMMAND": 0.08,
    "ACTUATOR_RESPONSE": 0.07,
    "WARNING_RAISED": 0.08,
    "ERROR_DETECTED": 0.07,
    "COMM_DELAY": 0.06,
    "NETWORK_TIMEOUT": 0.04,
    "DIAGNOSIS_START": 0.07,
    "RECOVERY_ACTION": 0.08,
    "RETRY_OPERATION": 0.06,
    "MAINTENANCE_CHECK": 0.04,
    "HEALTH_OK": 0.12,
}
...
EVENT_COMPONENT_WEIGHTS: Dict[str, Dict[str, float]] = {
    ...
    "VALUE_READ": {"SensorUnit": 1.0, "ProcessingUnit": 0.8, "ControlUnit": 0.3, "NetworkUnit": 0.2},
    "SETPOINT_UPDATE": {"ControlUnit": 1.0, "ProcessingUnit": 0.8, "SensorUnit": 0.2, "NetworkUnit": 0.2},
    "CONTROL_COMMAND": {"ControlUnit": 1.0, "ProcessingUnit": 0.7, "NetworkUnit": 0.4, "SensorUnit": 0.2},
    "ACTUATOR_RESPONSE": {"ControlUnit": 0.9, "ProcessingUnit": 0.6, "SensorUnit": 0.3, "NetworkUnit": 0.2},
    "WARNING_RAISED": {"SensorUnit": 0.8, "ControlUnit": 0.8, "NetworkUnit": 0.8, "ProcessingUnit": 0.8},
    "ERROR_DETECTED": {"SensorUnit": 0.8, "ControlUnit": 0.8, "NetworkUnit": 0.8, "ProcessingUnit": 0.8},
    "COMM_DELAY": {"NetworkUnit": 1.0, "ProcessingUnit": 0.6, "ControlUnit": 0.4, "SensorUnit": 0.2},
    "NETWORK_TIMEOUT": {"NetworkUnit": 1.0, "ProcessingUnit": 0.7, "ControlUnit": 0.5, "SensorUnit": 0.1},
    "DIAGNOSIS_START": {"ProcessingUnit": 1.0, "ControlUnit": 0.8, "NetworkUnit": 0.5, "SensorUnit": 0.4},
    "RECOVERY_ACTION": {"ControlUnit": 1.0, "ProcessingUnit": 0.8, "NetworkUnit": 0.6, "SensorUnit": 0.3},
    "RETRY_OPERATION": {"ControlUnit": 0.8, "NetworkUnit": 0.8, "ProcessingUnit": 0.7, "SensorUnit": 0.3},
    "MAINTENANCE_CHECK": {"SensorUnit": 0.7, "ControlUnit": 0.7, "NetworkUnit": 0.7, "ProcessingUnit": 0.7},
    "HEALTH_OK": {component: 1.0 for component in COMPONENTS},
}

MESSAGE_TEMPLATES: Dict[str, tuple[str, ...]] = {
    "STATUS_CHECK": (
        "{component} completed periodic status check; state={state}.",
        "{component} heartbeat accepted during {state} operation.",
        "Status poll from {component} returned {state}.",
    ),
    "VALUE_READ": (
        "{component} sampled process value while state={state}.",
        "Telemetry value received from {component} in {state} state.",
        "{component} measurement update committed.",
    ),
    "SETPOINT_UPDATE": (
        "{component} updated control setpoint during {state} state.",
        "Setpoint adjustment applied by {component}.",
        "{component} recalculated target value for current loop.",
    ),
    "CONTROL_COMMAND": (
        "{component} issued control command under {state} state.",
        "Control instruction dispatched from {component}.",
        "{component} command queue accepted new operation.",
    ),
    "ACTUATOR_RESPONSE": (
        "{component} received actuator response during {state} state.",
        "Actuator acknowledgement processed by {component}.",
        "{component} actuator feedback matched command window.",
    ),
    "WARNING_RAISED": (
        "{component} raised warning while state={state}.",
        "Warning threshold crossed on {component}.",
        "{component} reported noncritical operating deviation.",
    ),
    "ERROR_DETECTED": (
        "{component} detected abnormal condition during {state} state.",
        "Error signature observed by {component}.",
        "{component} flagged process deviation for review.",
    ),
    "COMM_DELAY": (
        "{component} observed communication delay while state={state}.",
        "Delayed packet sequence reported by {component}.",
        "{component} link latency exceeded nominal band.",
    ),
    "NETWORK_TIMEOUT": (
        "{component} reported network timeout during {state} state.",
        "Timeout waiting for peer response at {component}.",
        "{component} communication watchdog expired.",
    ),
    "DIAGNOSIS_START": (
        "{component} started diagnostic routine while state={state}.",
        "Diagnostic workflow initiated by {component}.",
        "{component} collected evidence for fault isolation.",
    ),
    "RECOVERY_ACTION": (
        "{component} applied recovery action during {state} state.",
        "Recovery procedure executed by {component}.",
        "{component} adjusted loop parameters for stabilization.",
    ),
    "RETRY_OPERATION": (
        "{component} retried operation while state={state}.",
        "Retry request accepted by {component}.",
        "{component} repeated operation after transient issue.",
    ),
    "MAINTENANCE_CHECK": (
        "{component} completed maintenance check during {state} state.",
        "Scheduled inspection record emitted by {component}.",
        "{component} maintenance validation passed local checks.",
    ),
    "HEALTH_OK": (
        "{component} reported healthy status while state={state}.",
        "Health signal from {component} remained nominal.",
        "{component} self-test completed without fault.",
    ),
}
...
HIDDEN_LABEL_COLUMNS: tuple[str, ...] = (
    "trajectory_id",
    "event_index",
    "segment_id",
    "behavior_id",
...
SYNIOL_BEHAVIOR_CATALOG: tuple[BehaviorSpec, ...] = (
    BehaviorSpec(
        behavior_id=0,
    ...
    BehaviorSpec(
        behavior_id=4,
        behavior_name="Normal Stable Operation",
        variants=(
            BehaviorVariantSpec(0, "normal_stable_a", {"risk_drift": -0.020, "health_recovery": 0.020, "load_smoothing": 0.10}),
            BehaviorVariantSpec(1, "normal_stable_b", {"risk_drift": -0.015, "health_recovery": 0.015, "load_smoothing": 0.12}),
            BehaviorVariantSpec(2, "normal_stable_c", {"risk_drift": -0.010, "health_recovery": 0.018, "load_smoothing": 0.08}),
        ),
    ),
)
...
class SynIOLGenerator:
    """Generate SynIOL trajectories from latent system dynamics and observable emissions."""

    def __init__(self, seed: int = 42) -> None:
        self.seed = seed
        self.rng = random.Random(seed)
        self.behaviors = {spec.behavior_id: spec for spec in SYNIOL_BEHAVIOR_CATALOG}
    ...
    def generate_dataset(
        ...
        """Generate a full SynIOL dataset with real transition regions and hidden metadata."""
    ...
    def build_metadata(
        ...
        behavior_counts["behavior_name"] = behavior_counts["behavior_id"].map(
            {spec.behavior_id: spec.behavior_name for spec in SYNIOL_BEHAVIOR_CATALOG}
        )
        ...
        return {
            ...
            "observable_columns": list(OBSERVABLE_COLUMNS),
            "hidden_label_columns": list(HIDDEN_LABEL_COLUMNS),
            "event_vocabulary": list(EVENT_VOCAB),
            "event_component_weights": EVENT_COMPONENT_WEIGHTS,
            "output_files": {
            ...
            "model_visibility_note": "Use syniol_observable.csv as model input. syniol_labels.csv contains latent/evaluation fields and must not be used as model input.",
    ...
    def save_dataset(self, dataset: pd.DataFrame, metadata: dict, output_dir: str | Path, output_name: str) -> dict:
        """Save observable logs, hidden labels, and metadata JSON."""

        output_dir = Path(output_dir)
        output_dir.mkdir(parents=True, exist_ok=True)
        observable_name = "syniol_observable.csv"
        labels_name = "syniol_labels.csv"
        full_name = "syniol_full.csv"
        if output_name != observable_name:
            observable_name = output_name
            labels_name = f"{Path(output_name).stem}_labels.csv"
            full_name = f"{Path(output_name).stem}_full.csv"
        observable_path = output_dir / observable_name
        labels_path = output_dir / labels_name
        full_path = output_dir / full_name
        metadata_path = output_dir / "syniol_manifest.json"
        description_path = output_dir / "syniol_description.md"

        dataset.loc[:, OBSERVABLE_COLUMNS].to_csv(observable_path, index=False)
        dataset.loc[:, HIDDEN_LABEL_COLUMNS].to_csv(labels_path, index=False)
        dataset.to_csv(full_path, index=False)
        metadata_path.write_text(json.dumps(metadata, indent=2), encoding="utf-8")
        description_path.write_text(self._build_dataset_description(metadata), encoding="utf-8")
        return {
            "observable_csv_path": str(observable_path),
    ...
    def get_behavior_catalog(self) -> list[dict]:
        ...
        return [asdict(spec) for spec in SYNIOL_BEHAVIOR_CATALOG]

    def _build_dataset_description(self, metadata: dict) -> str:
        behavior_lines = []
        for behavior in metadata["catalog"]:
            variants = ", ".join(variant["variant_name"] for variant in behavior["variants"])
            behavior_lines.append(f"- {behavior['behavior_id']}: {behavior['behavior_name']} ({variants})")

        event_lines = []
        for event_type in metadata["event_vocabulary"]:
            weights = metadata["event_component_weights"][event_type]
            strongest_components = sorted(weights, key=weights.get, reverse=True)
            event_lines.append(
                f"- {event_type}: strongest components: {', '.join(strongest_components[:2])}"
            )

        observable_columns = "\n".join(f"- {column}" for column in metadata["observable_columns"])
        hidden_columns = "\n".join(f"- {column}" for column in metadata["hidden_label_columns"])
        output_files = "\n".join(f"- `{name}`: {description}" for name, description in metadata["output_files"].items())
        behaviors = "\n".join(behavior_lines)
        events = "\n".join(event_lines)

        return f"""# SynIOL Dataset
...
- Events: {metadata["generated_num_events"]}
- Trajectories: {metadata["generated_num_trajectories"]}
- Events per trajectory: {metadata["events_per_trajectory"]}
- Transition events: {metadata["transition_event_count"]}
- Seed: {metadata["seed"]}
...
"""
    ...
    def _initialize_latent_state(self, plan: SegmentPlan, previous_state: dict | None) -> dict:
        ...
        if plan.behavior_id == 0:
            ...
            recovery = self.rng.uniform(0.00, 0.08)
            load_level = self._clip(0.6 * load_level + 0.4 * self.rng.uniform(0.25, 0.55))
            health = {component: self.rng.uniform(0.88, 0.99) for component in COMPONENTS}
    ...
    def _update_latent_state(
        ...
        if behavior_id == 0:
            ...
            load_smoothing = float(params["load_smoothing"])
            risk = risk + float(params["risk_drift"]) - 0.02 * recovery + load_effect + self.rng.uniform(-0.012, 0.012)
            recovery = max(0.0, recovery - 0.02 + self.rng.uniform(-0.01, 0.01))
            for component in COMPONENTS:
                health[component] = self._clip(health[component] + float(params["health_recovery"]) + self.rng.uniform(-0.008, 0.008))
            current["load_level"] = self._clip(current["load_level"] + load_smoothing * (load_anchor - current["load_level"]))
    ...
    def _emit_row(
        ...
        severity = self._observe_severity(state, event_type)
        ...
        return {
            ...
            "severity": severity,
            "state": state_name,
            "message": self._format_message(event_type, component, state_name),
    ...
    def _sample_component(
        ...
        if behavior_id == 3 and event_type in {"ERROR_DETECTED", "COMM_DELAY", "NETWORK_TIMEOUT"}:
    ...
    def _observe_severity(self, state: dict, event_type: str) -> str:
        ...
        index = self._adjust_severity_for_event(event_type, index)
        observed = SEVERITY_VOCAB[index]
        state["observed_severity"] = observed
        return observed

    def _adjust_severity_for_event(self, event_type: str, risk_index: int) -> int:
        allowed_by_event = {
            "STATUS_CHECK": (0, 1),
            "VALUE_READ": (0, 1),
            "SETPOINT_UPDATE": (0, 1),
            "CONTROL_COMMAND": (0, 1),
            "ACTUATOR_RESPONSE": (0, 1),
            "WARNING_RAISED": (0, 1, 2),
            "ERROR_DETECTED": (1, 2, 3),
            "COMM_DELAY": (0, 1, 2),
            "NETWORK_TIMEOUT": (1, 2),
            "DIAGNOSIS_START": (0, 1, 2),
            "RECOVERY_ACTION": (0, 1),
            "RETRY_OPERATION": (0, 1, 2),
            "MAINTENANCE_CHECK": (0, 1),
            "HEALTH_OK": (0, 1),
        }
        allowed = allowed_by_event[event_type]
        if risk_index in allowed and self.rng.random() > 0.12:
            return risk_index
        nearest = min(allowed, key=lambda candidate: abs(candidate - risk_index))
        if event_type in {"WARNING_RAISED", "ERROR_DETECTED", "COMM_DELAY", "NETWORK_TIMEOUT"} and self.rng.random() < 0.20:
            return self.rng.choice(allowed)
        return nearest

    def _format_message(self, event_type: str, component: str, state_name: str) -> str:
        template = self.rng.choice(MESSAGE_TEMPLATES[event_type])
        return template.format(component=component, state=state_name)
