...
This document describes the design of the `syniol` behavior-log dataset family used in the `behavior_log` experiment pipeline.

The goal of `syniol` is to move away from template-like synthetic logs and toward a more realistic, controllable, behavior-driven benchmark in which:
...
`syniol` contains four behavior classes:
...
`variant_id` in `syniol` is not a template variant. It is a latent-process variant.
...
`syniol` uses a mostly shared event vocabulary:
...
`syniol` uses a shared base event distribution and then applies only modest latent-conditioned adjustments.
...
Observed `state` in `syniol` is intentionally weakened relative to the latent process.
...
`syniol` keeps two numeric fields:
...
In the current `syniol` implementation, numeric values are meaningful but still relatively structured. This is one reason simple baselines such as PCA and summary statistics remain strong.
...
Transitions in `syniol` are real data, not just flags.
...
`syniol` includes moderate noise that does not change the label but makes the sequence less clean.
...
The `syniol` raw dataset is later converted to windows using:
...
- raw: `artifacts/datasets/syniol_moderate_noise/raw`
- preprocessed: `artifacts/datasets/syniol_moderate_noise/preprocessed`
- windows: `artifacts/datasets/syniol_moderate_noise/windows`
...
The raw `syniol` logs preserve evaluation-facing metadata that is not meant to be used directly for model training.
...
The current default `syniol` dataset configuration uses:
...
[generate_syniol.py](E:\thesis project\log-to-vec-benchmark\experiments\behavior_log\scripts\generate_syniol.py)

using:

[syniol.yaml](E:\thesis project\log-to-vec-benchmark\experiments\behavior_log\configs\datasets\syniol.yaml)

## Current Strengths

The strongest properties of the current `syniol` design are:
...
Even after weakening state, severity, and direct event-emission bias, `syniol` still contains strong learnable structure.
...
This suggests that the current `syniol` still exposes some structured shortcuts, most likely through:
...
That is not necessarily a flaw, but it means `syniol` is best viewed as:

> a more realistic and process-grounded synthetic benchmark, not yet a fully shortcut-resistant one.

## Recommended Interpretation

`syniol` should be understood as a benchmark that tests whether embeddings can capture:
...
At the same time, `syniol` remains controlled enough that we can diagnose where shallow baselines still succeed and where learned sequence models begin to add value.

## Files and Entry Points

Implementation:

- [syniol.py](E:\thesis project\log-to-vec-benchmark\experiments\behavior_log\src\behavior_log\generation\syniol.py)
...
- [datasets/syniol.yaml](E:\thesis project\log-to-vec-benchmark\experiments\behavior_log\configs\datasets\syniol.yaml)
- [preprocessing/syniol.yaml](E:\thesis project\log-to-vec-benchmark\experiments\behavior_log\configs\preprocessing\syniol.yaml)
- [windowing/syniol.yaml](E:\thesis project\log-to-vec-benchmark\experiments\behavior_log\configs\windowing\syniol.yaml)

Example generated artifacts:

- [syniol_observable.csv](E:\thesis project\log-to-vec-benchmark\experiments\behavior_log\artifacts\datasets\syniol\raw\syniol_observable.csv)
- [syniol_manifest.json](E:\thesis project\log-to-vec-benchmark\experiments\behavior_log\artifacts\datasets\syniol\raw\syniol_manifest.json)
- [windows.npz](E:\thesis project\log-to-vec-benchmark\experiments\behavior_log\artifacts\datasets\syniol\windows\windows.npz)
