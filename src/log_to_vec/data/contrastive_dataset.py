...
with shape (N, T, D), where:
    ...
    It supports different positive-pair construction strategies:
        ...
        - "identity": use the same sequence as both views
...
class ContrastiveDataset(Dataset):
    ...
    def __init__(
        ...
        if self.pair_mode not in ("neighbor", "augment", "identity"):
            raise ValueError(
                f"Unsupported pair_mode: {self.pair_mode}. "
                f"Use 'neighbor', 'augment', or 'identity'."
            )
